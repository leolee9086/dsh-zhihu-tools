	...
	factory: (require) => {
		...
		function ZhihuSettings() {
			...
			return react.createElement("div", { style: S.root },
				react.createElement("div", { style: S.notice }, "凭证仅驻留本进程内存,重启后需重新配置;不写入磁盘或日志。"),
		...
		function apply(ctx) {
			...
			ctx.slots.inject("tool.call.toolview", () => {
				const d1 = ctx.slots.register({ name: "tool.call.toolview", key: "zhihu_hot" }, ZhihuHotView);
				const d2 = ctx.slots.register({ name: "tool.call.toolview", key: "zhihu_search" }, (p) => react.createElement(ZhihuSearchView, { ...p, isGlobal: false }));
				const d3 = ctx.slots.register({ name: "tool.call.toolview", key: "zhihu_global_search" }, (p) => react.createElement(ZhihuSearchView, { ...p, isGlobal: true }));
				const d4 = ctx.slots.register({ name: "tool.call.toolview", key: "zhihu_ask" }, ZhihuAskView);
				return () => { d1(); d2(); d3(); d4(); };
			});
		}
		exports.apply = apply;
