...
// 凭据(Cookie、Access Secret)只驻留进程内存,卸载即清。

import { readFileSync } from "node:fs";

const name = "zhihu-tools-static";
const inject = ["tools", "webServer", "timer"];
    ...
    try {
      ...
      if (state.cookies.z_c0) {
        ...
        if (found) {
          ...
          state.qr = { ...state.qr, phase: "logged_in_with_secret", note: "二维码登录成功: " + found.path };
        } else {
          state.qr = { ...state.qr, note: "网页会话已建立,但未能自动取得 Access Secret;请在设置页粘贴密钥完成最后一步" };
        }
        void ui;
        return;
      }
...
function unwrapEnv(r) {
  ...
  return { ok: false, code, error: CODE_TEXT[code] ?? (msg || "平台错误码 " + code) };
}
...
// 开放平台对同一端点限频严格(实测:约 45 秒内对 hot_list 连打 4 发即触发 30001,
// 而 20001 才是鉴权失败):对 developer.zhihu.com 的调用经全局链串行化并保持最小间隔,
// 收到 30001 时退避后自动重试一次。www.zhihu.com 登录流不经过此门。
...
async function probe(state) {
  ...
  try {
    ...
    return {
      ...
      valid: u.ok === true,
      ...(u.ok ? {} : { platformCode: u.code ?? "无 JSON 响应" }),
...
function apply(ctx) {
  ...
  ctx.effect(() => () => {
    ...
    state.cookies = {};
  }, "zhihu-tools-static: purge credentials on dispose");
  ...
  const userTool = (toolName, description, extraProps, pathname, title) => register(toolDef(
    toolName,
    description,
    { type: "object", properties: { offset: { type: "string", description: "分页偏移量,翻页传 Paging.NextOffset" }, limit: { type: "number", description: "数量 1-50,默认 20" }, ...(extraProps || {}) }, additionalProperties: false },
    async function execute(args) {
      try {
        const q = {};
        if (str(args, "offset")) q.offset = str(args, "offset");
        const l = num(args, "limit", 20, 1, 50);
        if (l !== 20) q.limit = String(l);
        for (const k of Object.keys(extraProps || {})) if (str(args, k)) q[k] = str(args, k);
  ...
  userTool("zhihu_my_contents", "获取本人创作内容(回答/文章/视频/想法/问题),支持类型筛选、排序与分页。", { content_type: { type: "string", description: "all(默认)/answer/article/zvideo/pin/question" }, sort_field: { type: "string", description: "like_count/ts(默认)" }, sort_order: { type: "string", description: "asc/desc(默认)" } }, "/api/v1/user/contents", "我的创作");
  ...
  userTool("zhihu_favlist_contents", "获取本人指定收藏夹的内容(FavlistUrlToken 从 zhihu_my_favlists 获得)。", { favlist_url_token: { type: "string", description: "收藏夹 url_token" } }, "/api/v1/user/favlist_contents", "收藏夹内容");
