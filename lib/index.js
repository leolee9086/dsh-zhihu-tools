...
import { adaptForZhihu, markdownToZhihuHtml } from "./zhihu-markdown.js";
...
// 发布流水(窗口内每次发布的时间戳),用于在本地先挡一道官方限额。
const PUBLISH_LOG_KEY = "zhihu-tools-static/publish-log"; // CredentialKey: <scope>/<id>
// 官方 zhihu-publisher 的限额:每人每天最多发布 50 次。本地拦一道,免得撞平台的 429。
const DAILY_PUBLISH_LIMIT = 50;
const PUBLISH_WINDOW_MS = 24 * 3600 * 1000;
...
async function publishArticleViaSession(state, { title, html, commentPermission }) {
  ...
  // articleId 来自服务器响应,属于不可信输入,拼进 URL 前一律编码,避免路径穿越。
  const safeId = encodeURIComponent(String(articleId));
  const patched = await zhFetch(state, draftBase + "/" + safeId + "/draft", {
    method: "PATCH",
    headers: baseHeaders,
  ...
  const published = await zhFetch(state, draftBase + "/" + safeId + "/publish", {
    method: "PUT",
    headers: baseHeaders,
...
/** 读发布流水(窗口内每次发布的时间戳)。存储不可用时返回空数组,不阻塞发布。 */
async function readPublishLog(ctx) {
  const credentials = ctx.get("credentials");
  if (!credentials || typeof credentials.readRecord !== "function") return [];
  try {
    const rec = await credentials.readRecord(PUBLISH_LOG_KEY);
    const payload = rec && rec.kind === "grant" && rec.payload && typeof rec.payload === "object" ? rec.payload : null;
    const entries = payload && Array.isArray(payload.entries) ? payload.entries : [];
    return entries.filter((t) => typeof t === "number" && Number.isFinite(t));
  } catch {
    // 记录缺失或不可读,按「没有历史」处理。
    return [];
  }
...
/** 记一次发布,并丢掉窗口外的旧记录。写入失败只影响限额记账,不影响已经成功的发布。 */
async function notePublish(ctx, at) {
  const credentials = ctx.get("credentials");
  if (!credentials || typeof credentials.modifyRecord !== "function") return false;
  const kept = (await readPublishLog(ctx)).filter((t) => t > at - PUBLISH_WINDOW_MS);
  kept.push(at);
  try {
    await credentials.modifyRecord(PUBLISH_LOG_KEY, async () => ({
      kind: "grant",
      payload: { version: 1, entries: kept },
    }));
    return true;
  } catch {
    return false;
...
function apply(ctx) {
  ...
  ctx.effect(() => () => {
    ...
    // 发布密钥与网页会话同样只活在内存里,卸载时必须一并抹掉,不能留在闭包里。
    state.openapiKey = null;
    state.openapiSecret = null;
    state.openapiSource = null;
    state.cookies = {};
  ...
  ctx.on("tools/pre-execute", async (exec, next) => {
    // 用前缀匹配而不是精确名:将来新增 zhihu_publish_pin / zhihu_publish_question 时,
    // 它们会自动继承这道授权关,不会因为忘记登记而漏掉。
    if (!exec || typeof exec.name !== "string" || !exec.name.startsWith("zhihu_publish_")) return next();
    const args = exec.arguments || {};
    if (args.dry_run === true || args.confirm !== true) return next();
    // 标题来自调用方,进审批提示前先收敛:压掉空白并截断,避免有人靠它伪造提示文案。
    const rawTitle = typeof args.title === "string" ? args.title.replace(/\s+/g, " ").trim() : "";
    const title = rawTitle ? rawTitle.slice(0, 60) + (rawTitle.length > 60 ? "…" : "") : "(无标题)";
    return {
      kind: "ask",
  ...
  register(toolDef(
    ...
    "发布文章到知乎,支持两种授权:官方 Publish OpenAPI(需 ZHIHU_OPENAPI_APP_KEY/APP_SECRET)与网页会话(扫码登录后的 cookie)。必须显式传 confirm=true 才会真正发布。正文用 markdown 传(推荐),也可以用 html 直接给,两者至少给一个。",
    {
      type: "object",
      properties: {
        title: { type: "string", description: "文章标题(必填,不能为空)" },
        markdown: { type: "string", description: "文章正文 Markdown(推荐用这个)。渲染后会自动按知乎的口味修结构:表头行并进 tbody、知乎拆得坏的列表拍平成段落。与 html 二选一,给了 html 就以 html 为准。" },
        html: { type: "string", description: "文章正文 HTML(与 markdown 二选一)。同样会过一遍结构适配,所以不必自己把表头搬进 tbody。" },
    ...
    async function execute(args) {
      try {
        ...
        const rawHtml = str(args, "html");
        if (!title) return "ERROR: title 不能为空";
        if (!markdown.trim() && !rawHtml.trim()) {
          return "ERROR: 正文不能为空 —— 用 markdown 给(推荐),或者用 html 直接给。";
        }
        // 上限只为挡住明显异常的输入;知乎侧自己还有更严格的长度预检。
        if (title.length > 200) return "ERROR: title 过长(" + title.length + " 字符,上限 200)";

        // 正文:markdown 交给 lute 渲染,html 直接用。两条路都过一遍知乎结构适配
        // (表头行并进 tbody、知乎拆得坏的列表拍平)——适配是幂等的,所以已经处理过的
        // html 再走一遍也不会变样。
        const html = rawHtml.trim()
          ? adaptForZhihu(rawHtml)
          : await markdownToZhihuHtml(markdown);
        if (!html.trim()) return "ERROR: 正文转换后是空的";
        if (html.length > 1000000) return "ERROR: 正文过长(" + html.length + " 字符,上限 1000000)";

        // 枚举值在这里再校验一次:schema 的 enum 由平台把关,但工具本身不该依赖调用方一定走过 schema。
        const COMMENT_PERMISSIONS = ["all", "nobody", "followee", "censor", "follower"];
        const commentPermission = str(args, "comment_permission", "all") || "all";
        if (!COMMENT_PERMISSIONS.includes(commentPermission)) {
          return "ERROR: comment_permission 取值非法: " + commentPermission + " (可选 " + COMMENT_PERMISSIONS.join(" / ") + ")";
        }
        const CREATION_STATEMENTS = ["spoiler", "medical_advice", "fictional_creation", "contain_finance", "ai_creation"];
        const statement = str(args, "creation_statement");
        if (statement && !CREATION_STATEMENTS.includes(statement)) {
          return "ERROR: creation_statement 取值非法: " + statement + " (可选 " + CREATION_STATEMENTS.join(" / ") + ")";
        }
        ...
        const AUTH_MODES = ["auto", "openapi", "session"];
        const requested = str(args, "auth_mode", "auto") || "auto";
        if (!AUTH_MODES.includes(requested)) {
          return "ERROR: auth_mode 取值非法: " + requested + " (可选 " + AUTH_MODES.join(" / ") + ")";
        }
        ...
        // 官方限额:zhihu-publisher 每人每天最多发布 50 次。先在本地挡一道,免得撞平台的 429。
        const publishLog = await readPublishLog(ctx);
        const cutoff = Date.now() - PUBLISH_WINDOW_MS;
        const usedInWindow = publishLog.filter((t) => t > cutoff).length;
        if (usedInWindow >= DAILY_PUBLISH_LIMIT) {
          return "ERROR: 已达官方发布上限(" + DAILY_PUBLISH_LIMIT + " 次 / " + (PUBLISH_WINDOW_MS / 3600000)
            + " 小时,窗口内已发 " + usedInWindow + " 次)。请等窗口滚动后再试,不要反复重试,以免被判定为违规调用。";
        }
        ...
        const preview = {
          ...
          quota: {
            ...
            windowHours: PUBLISH_WINDOW_MS / 3600000,
            remaining: DAILY_PUBLISH_LIMIT - usedInWindow,
          },
          openapi: {
            configured: !!creds,
        ...
        if (res.ok) {
          await notePublish(ctx, Date.now()); // 记账失败不影响本次已成功的发布
          return jsonText({
            published: true,
            authMode: mode,
            url: res.url,
