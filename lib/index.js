...
function apply(ctx) {
  ...
  // 个人数据类工具:实测平台要求 PascalCase 查询参数(ContentType/FavlistUrlToken/offset 等),
  // 小写驼峰会报 Code=10001 "xxx is required"。参数名→线上名映射见各 extraQuery。
  const userTool = (toolName, description, extraProps, pathname, title, extraQuery) => register(toolDef(
    toolName,
    description,
  ...
  userTool("zhihu_my_contents", "获取本人创作内容(回答/文章/视频/想法/问题),支持类型筛选、排序与分页。ContentType 必填。", { content_type: { type: "string", description: "answer(默认)/article/zvideo/pin/question" } }, "/api/v1/user/contents", "我的创作", { content_type: "ContentType" });
  ...
  userTool("zhihu_favlist_contents", "获取本人指定收藏夹的内容(FavlistUrlToken 从 zhihu_my_favlists 获得,必填)。", { favlist_url_token: { type: "string", description: "收藏夹 url_token(必填)" } }, "/api/v1/user/favlist_contents", "收藏夹内容", { favlist_url_token: "FavlistUrlToken" });
